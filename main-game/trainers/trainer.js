        ...
        if (itemData.type === 'ball') {
            ...
            if (capturedPokemon) {
                ...
                if (this.belt.length < 6) {
                    ...
                    console.log(`You already have 6 Pokemon! You will have to send one Pokemon to your PC storage.`)
                }
                ...
                pokemonChoices.unshift(`${capturedPokemon.name}: Level ${capturedPokemon.level} | HP - ${capturedPokemon.hitPoints.current}/${capturedPokemon.hitPoints.max} - new`)
                ...
                },
                {
                    ...
                    when: (answers) => {
                        ...
                        if (answers.pokemonName) {
                            pokemonChoices[0] = pokemonChoices[0].replace(capturedPokemon.name, answers.pokemonName)
                        }
                ...
                .then((answers) => {
                    ...
                    if (answers.pokemonToStore) {
                        ...
                        const selectedPokemonIndex = this.getPokemon(selectedPokemon).index
            ...
            target.hitPoints.current = +Math.min(target.hitPoints.max, target.hitPoints.current + effect.modifier).toFixed(2)
            console.log(`${target.name} regained ${effect.modifier} hit points! ${target.name} now has ${target.hitPoints.current}/${target.hitPoints.max} HP`)
            ...
            : 'rose sharply!'
            const statAfterMod = +modBy50(target[effect.stat.max].current, effect.modifier).toFixed(2)
    
            if (statAfterMod >= target[effect.stat.max] * 1.5 || statAfterMod >= 500) {
                console.log (`${target.name}'s ${effect.stat} is already too high. ${item} had no effect!`)
            }
            else {
...
// const tim = create.pokemon('Rattata', 'Tim', 1)
// const bim = create.pokemon('Rattata', 'Bim', 1)
// const sim = create.pokemon('Rattata', 'Sim', 1)
// const dim = create.pokemon('Rattata', 'Dim', 1)
// const glim = create.pokemon('Rattata', 'Glim', 1)
// const mim = create.pokemon('Rattata', 'Mim', 1)

// const tom = new Trainer('Tom', [tim,bim,sim,dim,glim,mim])

// const bobby = create.pokemon('Squirtle', 'Bobby', 1)
// bobby.takeDamage(20)

// tom.buyItem('Poke Ball')
// console.log(tom.inventory)

// tom.resolveItem('Poke Ball', bobby)
// .then((captured) => {
//     console.log(captured)
//     console.log(tom.pokemonList)
// })
